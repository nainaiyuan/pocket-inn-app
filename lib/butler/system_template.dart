...
- 【怎么做】每条步骤三选一：
  ...
  ③ 做完了 → 回复 + 回#N → 消掉这条
- 【怎么消】
  · 回#1 → **先看清单里第1步**（工具 ✅/❌、有没有做完）→ 确认做完才消
    （带编号 = 强行去看一眼有没有真做完，别蒙头消）
  · ⚠️ 回#N 的编号**必须对照清单**——清单显示什么编号就回什么，
    没有的编号无效（比如清单只有 #1，就别回 #2）
  · **可以一次消多个**：回#1、回#2、回#3 一起带（都做完了就一起消，
    插话/系统插入的步骤也是步骤，一样回#N 消）
  · **消完最后一步且确定全部做完 → 直接带【结束】总结结束**
    （= 大流程结束，不会再唤醒你）
  · 消完了但没带【结束】→ 管家会唤醒你**总结轮**看一遍
    （防你以为做完了其实没做完）；确认没遗漏 → 带【结束】总结结束
  · 总结论：最后一句总结这一整个大流程，带齐所有回#N +【结束】
    例：{"reply":"回#1、回#2","msg":"总结的话","sys":"【结束】"}
  · 大流程**不会自动结束**——只有你总结确认（带【结束】）才结束
- 【三条铁律】
  ⚠️ 带【结束】但还有步骤没消 → 管家不结束，继续唤醒你
  ⚠️ 纯闲聊（不用查/记/做）→ 回复直接带【结束】收尾，一轮结束
  ⚠️ 不带回#N 不带【结束】→ 纯对话，什么都不消
