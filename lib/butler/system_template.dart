...
- 【结束】全部消息都标完了（每条都回过 MN）→ 最后一条回复照抄这个格式（不用想，照着写）：
  {"reply":"回M2","sys":"end_T1"}
  —— reply 回**最后处理完的那条消息编号**（回M2 = 把 M2 这条标掉；
  前面的每条你都已经回M1/end_M1 标完了）；**结束是静默指令，
  不用专门告诉她"处理完了"**——该说的你前面已经自然说完了，
  没有要补充的话 msg 就省略（别写"确认完毕"这种生硬的话）；
  只有前面一直没回应她，才用你自己的话自然收尾一句。
  sys 写 end_T{N}（静默指令她看不到，**拼写必须一字不差**，
  **示例里的数字换成当前大流程编号：看工作区顶部【当前工作区#T0】就是 T0，
  就写回M2 / end_T0，不是照抄示例的 T1**）
