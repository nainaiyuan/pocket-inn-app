...
要调用就真正发出 function calling，不要只说不做。工具：
- record_memory：记住用户的喜好/习惯/约定/个人信息
...
不要逐条回复历史。''';
  ...
  /// ── 测试指令（用户 8-03 04:2x）──
  /// 测试时通过 taskState 占位注入（不写进 SYSTEM_CORE，测完即撤）。
  /// 直接列测试项让男主执行，不需要用户一步步引导。
  /// 改测试项只改这里的文本，代码不用动。
  static const String testInstruction = '''
【测试】现在进入测试模式，直接执行以下测试项，每项执行完把结果按人设自然说出来：
1. 回复格式测试：说一句符合人设的日常问候。要求：口语化、自然、不超过 200 字、无任何格式化内容（不要 JSON/列表/编号）
2. 工具调用测试：等用户说"记住我喜欢喝美式咖啡"后，真正调用 record_memory 工具（function calling），执行完告诉用户记住了
3. 记忆查询测试：等用户说"我之前说过喜欢什么吗"后，调用 recall_memory 工具查证，查到就回答，查不到就说没找到（不要编造）
4. 全部完成后说"测试完成"
''';
