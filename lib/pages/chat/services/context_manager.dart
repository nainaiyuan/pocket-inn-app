...
class ContextManager {
  ...
  /// - 原文取走即清空（被摘要替换）。
  /// - 8-11 06:3x（用户：男主总结后全忘了调过什么工具）：工具行不再扔掉——
  ///   工具记录（时间+工具名+成败+结果）跟着原文进总结，男主写摘要时可整理
  ///   成"工具使用经验"，或 keep_nos 保留工具行原文。男主不能忘自己调过什么
  ///   （8-04 17:0x 用户：上下文要留地方放工具，男主才知道做过什么）。
  ///   （8-05 19:19 曾定稿"工具/管家历史不重要就扔掉"——用户实测发现男主
  ///   总结后失忆，改回保留。）
  (int, int, String) takePendingRawWithRange(String personaId) {
    final t = _topics.remove(personaId);
    if (t == null) return (0, 0, '');
    final chatLines = List<String>.from(t.raw);
  ...
  String buildResyncContext(String personaId) {
    ...
    // 8-10 v3（5.6）：再次唤醒 → 提示整理——男主自己决定保留什么
    sb.write('\n\n【系统提示】刚唤醒，上下文可能旧了：整理一下——'
        '历史里还有用的写"历史#N~#M 还有用"保留原文，'
        '其他用自己的话整理成短期/长期记忆摘要；工具使用经验也这时整理。'
        '没处理的流程管家已保存，下次继续。');
    return sb.toString();
  }
